/*
Matrix Vector multiplication with to small cuda allocation of the matrix.
*/
#include <stdbool.h>
#include <stdio.h>
#include <stdlib.h>
...
int initcuda(){
    //Allocation of GPU memory for d_a,d_c and d_b at the size of an array instead of a matrix
    cudaErrorCheck( cudaMalloc(&d_a, C*sizeof(int)));
    cudaErrorCheck( cudaMalloc(&d_b, C*sizeof(int)));
