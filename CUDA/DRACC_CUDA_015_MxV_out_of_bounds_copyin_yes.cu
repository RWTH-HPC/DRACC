/*
Matrix Vector multiplication with copying more data in from one vector than allocated.
*/
#include <stdbool.h>
#include <stdio.h>
#include <stdlib.h>
...
int initcuda(){
    ...
    //Copying the two times array a and the matrix b from the host to the array d_a and the matrix d_b on the device
    cudaErrorCheck( cudaMemcpy(d_a,a,2*C*sizeof(int),cudaMemcpyHostToDevice));
