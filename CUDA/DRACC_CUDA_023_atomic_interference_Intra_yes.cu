/*
Incrementaion with a threshold of N. The incrementation is atomic per block, but the query on the threshold depends on the counter.
The query is not atomic, therefore an Intra Region Data Race is possible in line 30 and 31. At the end of the Kernel an Inter Region Data Race is induced by CUDAs memory model.
*/
#include <stdio.h>
